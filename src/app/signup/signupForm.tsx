...
import type { ISignUpDTO } from "@/types/auth.type";
...
export default function SignUpForm({
  ...
  signUpAction: (data: ISignUpDTO) => void;
}) {
  const [openCompany, setOpenCompany] = useState(false);
  const [companys, setCompanys] = useState<Array<{ id: number; name: string }>>(
    []
  );

  useEffect(() => {
    async function fetchCompanys() {
      let res = await fetch(`${process.env.API_URL}/company/list`);
  ...
  const handleSignUpAction = (values: z.infer<typeof formSchema>) => {
    signUpAction(values);
  };
  ...
  return (
    <div className="flex justify-center items-center h-full">
      <Card className="m-3 w-[400px]">
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleSignUpAction)}
            className="space-y-1"
          >
