...
import type { ITicketDTO } from "@/types/ticket.type";
import { statuses } from "@/data/status";

export default function TicketForm({
  ticketAction,
}: {
  ticketAction: (data: ITicketDTO) => void;
}) {
  ...
  const handleTicketAction = (values: z.infer<typeof ticketFormSchema>) => {
    ticketAction(values);
  }
  ...
  return (
    <div className="container h-full flex-1 flex-col space-y-3 md:flex px-24">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleTicketAction)}>
