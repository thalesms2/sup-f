...
export default function TicketCreate() {
	...
	const [companys, setCompanys] = React.useState<Array<{id: number, name: string, User: Array<{ id: number, username: string }> }>>([])
	...
	React.useEffect(() => {
		async function updateUsers(companyId: string) {
			const arr = companys.find((company) => company.id.toString() === companyId)?.User
			if(arr != undefined) {
				setUsers(arr)
			}
		}
	...
	async function onSubmit(values: z.infer<typeof ticketFormSchema>) {
		...
		values.actions = values.actions.filter((action) => action.description != "")
							...
							render={({ field }) => (
								<FormItem className="col-span-3">
									<Select onValueChange={field.onChange}>
										...
										<SelectContent>
											{statuses.map((status: { value: string, label: string, icon: any }) => {
												return (
